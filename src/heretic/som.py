...
import numpy as np
from minisom import MiniSom  # ty:ignore[unresolved-import]
...
class SOMCalculator:
    ...
    def __init__(
        self, som_x: int, som_y: int, iterations: int, lr: float, sigma: float
    ) -> None:
        """
    ...
    def get_top_k_neuron_weights(self, k: int) -> np.ndarray:
        ...
        winners = np.array(
            [
                self.som.winner(x)
                for x in self.som._weights.reshape(-1, self.som._weights.shape[2])
            ]
        )
        ...
        top_k_weights = np.array(
            [self.som.get_weights()[i, j] for i, j in top_k_coords]
        )

...
def run():
    ...
    if settings.multidirectional_som:
        if importlib.util.find_spec("minisom") is None:
            print(
                "Self-Organizing Map is selected, but 'minisom' module not installed.\nPlease install it with 'pip install minisom'."
            )
            return
    ...
    if settings.response_prefix:
        # Use the explicitly configured response prefix.
        model.response_prefix = settings.response_prefix
        print(f"Using configured response prefix: [bold]{model.response_prefix!r}[/]")
    else:
    ...
    if settings.multidirectional_som:
        ...
        bad_means = torch.stack(bad_means, dim=0).permute(
            1, 0, 2
        )  # N_directions, N_layers, hidden_dim
    else:
    ...
    if settings.orthogonalize_direction:
        ...
        projection_vectors = [
            torch.sum(ref * good_directions, dim=1) for ref in refusal_directions
        ]
        refusal_directions = [
            (refusal_direction - projection_vector.unsqueeze(1) * good_directions)
            for (refusal_direction, projection_vector) in zip(
                refusal_directions, projection_vectors
            )
        ]
        refusal_directions = [
            F.normalize(refusal_direction, p=2, dim=1)
            for refusal_direction in refusal_directions
        ]

    refusal_directions = torch.stack(refusal_directions, dim=0)
    refusal_directions = refusal_directions.permute(
        1, 0, 2
    )  # layers, directions, hidden_dim
        ...
        for component in model.get_abliterable_components():
            ...
            parameters[component] = AbliterationParameters(
                ...
                min_weights=[
                    ...
                    for (min_weight, max_weight) in zip(min_weights, max_weights)
                ],
